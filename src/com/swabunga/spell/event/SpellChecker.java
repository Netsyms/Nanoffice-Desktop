...
public class SpellChecker {
     ...
     * found. The number of errors are those that are found BEFORE and
     * corretions are made.
     */
    public final int checkSpelling(WordTokenizer tokenizer) {
        ...
        while (tokenizer.hasMoreWords() && !terminated) {
            ...
            if (!dictionary.isCorrect(word)) {
                if ((config.getBoolean(Configuration.SPELL_IGNOREMIXEDCASE) && isMixedCaseWord(word, tokenizer.isNewSentance()))
    ...
    /**
     * Added to free up the class memory and resources, which otherwise trash
     * the system quickly (code by Steve Birmingham)
     */
    public void dispose() {
        eventListeners = null;
        ignoredWords = null;

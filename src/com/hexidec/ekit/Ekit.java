...
import com.netsyms.nanoffice.desktop.NanofficeDesktop;
import com.netsyms.nanoffice.desktop.PrefStorage;
...
public class Ekit extends JFrame implements WindowListener {
     ...
     * Master Constructor
     *
     * @param sDocument [String] A text or HTML document to load in the editor
     * upon startup.
     * @param sStyleSheet [String] A CSS stylesheet to load in the editor upon
     * startup.
     * @param sRawDocument [String] A document encoded as a String to load in
     * the editor upon startup.
     * @param urlStyleSheet [URL] A URL reference to the CSS style sheet.
     * @param includeToolBar [boolean] Specifies whether the app should include
     * the toolbar.
     * @param showViewSource [boolean] Specifies whether or not to show the View
     * Source window on startup.
     * @param showMenuIcons [boolean] Specifies whether or not to show icon
     * pictures in menus.
     * @param editModeExclusive [boolean] Specifies whether or not to use
     * exclusive edit mode (recommended on).
     * @param sLanguage [String] The language portion of the
     * Internationalization Locale to run Ekit in.
     * @param sCountry [String] The country portion of the Internationalization
     * Locale to run Ekit in.
     * @param base64 [boolean] Specifies whether the raw document is Base64
     * encoded or not.
     * @param debugMode [boolean] Specifies whether to show the Debug menu or
     * not.
     * @param useSpellChecker [boolean] Specifies whether to include the
     * spellchecker or not.
     * @param multiBar [boolean] Specifies whether to use multiple toolbars or
     * one big toolbar.
     * @param enterBreak [boolean] Specifies whether the ENTER key should insert
     * breaks instead of paragraph tags.
     */
    public Ekit(String sDocument, String sStyleSheet, String sRawDocument, URL urlStyleSheet, boolean includeToolBar, boolean showViewSource, boolean showMenuIcons, boolean editModeExclusive, String sLanguage, String sCountry, boolean base64, boolean debugMode, boolean useSpellChecker, boolean multiBar, boolean enterBreak) {
        setIconImages(NanofficeDesktop.appicon);
        ...
        this.setSize(
                Integer.valueOf(
                        PrefStorage.getSetting("framewidth",
                                "" + this.getWidth()
                        )
                ),
                Integer.valueOf(
                        PrefStorage.getSetting("frameheight",
                                "" + this.getWidth()
                        )
                )
        );
        this.setLocation(
                Integer.valueOf(
                        PrefStorage.getSetting("framexpos",
                                "" + this.getLocationOnScreen().x
                        )
                ),
                Integer.valueOf(
                        PrefStorage.getSetting("frameypos",
    ...
    public static void exitPrompt(Frame c) {
        int p = JOptionPane.showConfirmDialog(c,
                "Are you sure you want to exit Nanoffice?",
                "Exit Nanoffice",
                JOptionPane.YES_NO_OPTION,
                JOptionPane.QUESTION_MESSAGE);
        if (p == JOptionPane.YES_OPTION) {
            if (c.getExtendedState() == MAXIMIZED_BOTH) {
                PrefStorage.saveSetting("framemaxed", "yes");
            } else {
                PrefStorage.saveSetting("framemaxed", "no");
                PrefStorage.saveSetting("framewidth", String.valueOf(c.getWidth()));
                PrefStorage.saveSetting("frameheight", String.valueOf(c.getHeight()));
                PrefStorage.saveSetting("framexpos", String.valueOf(c.getLocationOnScreen().x));
                PrefStorage.saveSetting("frameypos", String.valueOf(c.getLocationOnScreen().y));
            }
            PrefStorage.save();
            c.dispose();
            System.exit(0);
